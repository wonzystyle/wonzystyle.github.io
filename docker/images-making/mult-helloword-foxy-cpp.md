...
RUN git clone https://ghproxy.com/github.com/wonzystyle/helloword-foxy-cpp.git
...
#ARG ROS_DISTRO=foxy
# 编译
WORKDIR /helloword-foxy-cpp
RUN colcon build --packages-select talkandlisten
# RUN . /helloword-foxy-cpp/install/setup.bash
...
### 编译Dockerfile
```
DOCKER_BUILDKIT=1 docker buildx build --platform linux/arm64,linux/amd64 -t wonzystyle/helloword-foxy-cpp . --push
```
### 退出Docker
```
docker logout
